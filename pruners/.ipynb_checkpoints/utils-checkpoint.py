...
from L0_card import Heuristic_CD_PP,Active_IHTCDLS_PP,Heuristic_LS,Heuristic_LSBlock,evaluate_obj
from FLOP_solver import generate_weight, FLOP_solve, WFbeta_solve, WFbeta2_solve, CAIE1_solve
from group_solver import group_pruner
import L0_card_pytorch as torch_algo
...
def generate_schedule(num_stages, base_level,sparsity_level,schedule,repeat=1):
    if num_stages == 1:
...
def num_filters(model):
    total_filters_model = 0
    pruned_filters_model=0
    for name, layer in model.named_modules():
        if isinstance(layer, torch.nn.Conv2d):
            is_pruned_filter = torch.linalg.norm(torch.linalg.norm(layer.weight,dim=(2,3)),dim=1)
            pruned_filters = (is_pruned_filter==0).sum().item()
            total_filters =  is_pruned_filter.shape[0]
            print(name,total_filters,pruned_filters)
            total_filters_model +=total_filters
            pruned_filters_model+=pruned_filters
    return pruned_filters_model,total_filters_model


def propagate_sparsity(model,prune_dataloader, criterion, samples=100, device = "cuda"):
    params_norms_filters = {name:None for name,param in model.named_parameters()}
    params_norms_channels = {name:None for name,param in model.named_parameters()}
    for i, batch in enumerate(prune_dataloader):
        #if i%100 ==0:
        ...
        for name,param in model.named_parameters():
            if len(param.grad.shape) != 4:
                continue
            filters_norms = torch.linalg.vector_norm(param.grad,dim=(1,2,3))
            channels_norms = torch.linalg.vector_norm(param.grad,dim=(0,2,3))
            if params_norms_filters[name] is None:
                params_norms_filters[name] = filters_norms
            else:
                params_norms_filters[name] += filters_norms
            
            if params_norms_channels[name] is None:
                params_norms_channels[name] = channels_norms
            else:
                params_norms_channels[name] += channels_norms
    ...
    for name,param in model.named_parameters():
        if len(param.shape) != 4:
            continue
        param.data[(params_norms_filters[name] == 0),:,:,:] = 0
        param.data[:,(params_norms_channels[name] == 0),:,:] = 0
    zero_grads(model)

def num_channels(model):
    total_channels_model = 0
    pruned_channels_model=0
    for name, layer in model.named_modules():
        if isinstance(layer, torch.nn.Conv2d):
            is_pruned_channel = torch.linalg.norm(torch.linalg.norm(layer.weight,dim=(2,3)),dim=0)
            pruned_channels = (is_pruned_channel==0).sum().item()
            total_channels =  is_pruned_channel.shape[0]
            print(name,total_channels,pruned_channels)
            total_channels_model +=total_channels
            pruned_channels_model+=pruned_channels
    return pruned_channels_model,total_channels_model
...
def model_factory(arch,dset_path,pretrained=True):
    if arch == 'mlpnet':
        ...
        state_trained = torch.load('../WoodFisher/checkpoints/resnet20_cifar10.pth.tar',map_location=torch.device('cpu'))['state_dict']
        new_state_trained = OrderedDict()
        for k in state_trained:
        ...
        if train_random_transforms:
            ...
            train_transform = transforms.Compose([transforms.ToTensor(),
                                                transforms.Normalize(means, stds)])
        train_dataset = datasets.CIFAR100(root=dset_path, train=True,
                                        download=True, transform=train_transform)

        test_transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(means, stds)])

        test_dataset = datasets.CIFAR100(root=dset_path, train=False,
        ...
        model = model = torch.hub.load('pytorch/vision:v0.10.0', 'vgg11', pretrained=pretrained)
        train_dataset,test_dataset = imagenet_get_datasets(dset_path)
        ...
        state_trained = torch.load('../pytorch-vgg-cifar10/save_vgg16_bn/checkpoint_299.tar',map_location=torch.device('cpu'))['state_dict']
        new_state_trained = OrderedDict()
        for k in state_trained:
            if 'module' in k:
                key = k.split('.module.')[0] +'.' + k.split('.module.')[1] 
            else:
                key = k
        ...
        for name, param in model.named_parameters():
            ...
            #print("name is {} and shape of param is {} \n".format(name, param.shape))
            layer_name,param_name = '.'.join(name.split('.')[:-1]),name.split('.')[-1]
            #print(layer_name,param_name)
            if param_name == 'bias':
                    continue
            if 'conv' in layer_name or 'fc' in layer_name or 'downsample.0' in layer_name:
                modules_to_prune.append(name)
        if pretrained:
