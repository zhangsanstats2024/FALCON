...
def FLOP_solve(y,grads,w1,k,flop_k,alpha,lambda1,lambda2,M, beta_tilde1,
                    beta_tilde2, L=None, iht_max_itr=100, ftol = 1e-7, act_max_itr=10, buget=None, kimp=1.5, act_itr=1,
                    cd_itr = 0, ctol = 1e-4, sea1_max_itr=5, sea2_max_itr=10):
    
    # generate support
    
    v_w = -beta**2
    ind_mask, tot_iter = iht_multi(v_w, f_w, k, flop_k, 1e-12, flopmodel)
    s_cur = len(np.where(ind_mask)[0])
    beta *= ind_mask
